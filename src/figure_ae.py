...
for f in files:
    ...
    sns.lineplot(moving_average(rte.rot), ax=ax[0], label=exp.params.name, alpha=0.5)
    sns.lineplot(moving_average(vel), ax=ax[1], label="vel", alpha=0.5)
    sns.lineplot(moving_average(rte.trans), ax=ax[1], label=exp.params.name, alpha=0.5)
