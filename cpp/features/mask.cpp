...
bool markOutOfRangeInvalid(const size_t &idx, const double &point_range,
                           ...
                           const FeatureExtractionParams &params,
                           std::vector<bool> &mask) {
  if (point_range < lidar_params.min_range ||
      point_range > lidar_params.max_range) {
    mask[idx] = false;
...
bool markOccludedInvalid(const size_t &idx, const double &point_range,
  ...
  if (next_point_range - point_range > params.occlusion_thresh) { // Case 1
    for (size_t n = 1; n <= params.neighbor_points; n++)
      mask[idx + n] = false;
    return true;
  } else if (point_range - next_point_range >
             params.occlusion_thresh) { // Case 2
...
bool markParallelInvalid(const size_t &idx, const double &prev_point_range,
  ...
  if (diff_next > params.parallel_thresh * point_range &&
      diff_prev > params.parallel_thresh * point_range) {
    mask[idx] = false;
    return true;
  }
...
void makeMask(py::module &m) {
  m.def("computeValidPoints", &computeValidPoints<FieldAccessor, Point>,
        "Compute the valid points in a LiDAR scan");
}