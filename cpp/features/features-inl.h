...
extractFeatures(const std::vector<PointType, Alloc<PointType>> &input_scan,
  ...
  std::vector<PointCurvature> curvature =
      computeCurvature<Accessor>(input_scan, lidar_params, params);
  ...
  for (size_t scan_line_idx = 0; scan_line_idx < lidar_params.num_rows;
    ...
    for (size_t sector_idx = 0; sector_idx < params.number_sectors;
      ...
      std::sort(curvature.begin() + sector_start_pt,
                curvature.begin() + sector_end_pt, curvatureComparator);

      // Search largest to smallest curvature [i.e. edge features] WARN: Mutates
      // out_features + valid_mask
      features_internal::extractSectorEdgeFeatures(
          sector_start_pt, sector_end_pt, input_scan, curvature, params,
          out_features, valid_mask);
      // Search smallest to largest [i.e. planar features] WARN: Mutates
      // out_features + valid_mask
      features_internal::extractSectorPlanarFeatures(
          sector_start_pt, sector_end_pt, input_scan, curvature, params,
          out_features, valid_mask);
  ...
  for (size_t scan_line_idx = 0; scan_line_idx < lidar_params.num_rows;
    ...
    for (size_t line_pt_idx = 0; line_pt_idx < lidar_params.num_columns;
      ...
      // If not an edge point compute the curvature
        ...
        double dx = -(2.0 * params.neighbor_points) *
                    Accessor<PointType>::x(input_scan[idx]);
        double dy = -(2.0 * params.neighbor_points) *
                    Accessor<PointType>::y(input_scan[idx]);
        double dz = -(2.0 * params.neighbor_points) *
                    Accessor<PointType>::z(input_scan[idx]);
        // Iterate over neighbors and accumulate
        for (size_t n = 1; n <= params.neighbor_points; n++) {
          dx = dx + Accessor<PointType>::x(input_scan[idx - n]) +
               Accessor<PointType>::x(input_scan[idx + n]);
          dy = dy + Accessor<PointType>::y(input_scan[idx - n]) +
               Accessor<PointType>::y(input_scan[idx + n]);
          dz = dz + Accessor<PointType>::z(input_scan[idx - n]) +
               Accessor<PointType>::z(input_scan[idx + n]);
        }
...
void extractSectorEdgeFeatures(
  ...
  for (size_t sorted_curv_idx_p1 = sector_end_point;
    ...
    const PointCurvature curv =
        curvature[sorted_curv_idx_p1 -
                  1]; // subtraction as loop cannot go negative

    if (valid_mask[curv.index] && curv.curvature > params.edge_feat_threshold) {
      out_features.edge_points.push_back(
          input_scan.at(curv.index)); // Add to edge points
    ...
    if (num_sector_edge_features > params.max_edge_feats_per_sector)
      break;
  }
}
...
void extractSectorPlanarFeatures(
  ...
  for (size_t sorted_curv_idx = sector_start_point;
    ...
    if (valid_mask[curv.index] &&
        curv.curvature < params.planar_feat_threshold) {
      out_features.planar_points.push_back(
          input_scan.at(curv.index)); // Add to edge points
      for (size_t n = 0; n < params.neighbor_points; n++) { // update mask
        valid_mask[curv.index + n] = false;
        valid_mask[curv.index - n] = false;
    ...
    // Early exit if we have found enough features
    if (num_sector_planar_features > params.max_planar_feats_per_sector)
      break;
